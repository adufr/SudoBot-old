...
module.exports = class extends Command {
  constructor (...args) {
    ...
    this.usageCustom = 'sudo help <command>'
    this.example = 'sudo help info'
  }
  ...
  async run (message) {
    ...
    if (args[1]) return message.reply(`commande **sudo ${args[1]}** introuvable... Tapez \`sudo help\` pour afficher la liste des commandes.`)
    ...
    Promise.all([
      ...
      const embed = new MessageEmbed()
        ...
        .addField('Commandes générales :', responses[0])
        .addField('Commandes configuration :', responses[1])
        .setTimestamp()
  ...
  return new Promise(function (resolve, reject) {
    fs.readdir(commandsPath, (err, files) => { // loops on each folder's file
      ...
      jsFiles.forEach((f, i) => {
        ...
        if (!command.hidden) {
          description += `\`su ${client.funcs.beautify(`${command}`, 15)}\` - ${command.description}\n`
        }
      })
      resolve(description)
    })
